...
bool CameraSensorHandler::onInitialize(rclcpp::Node::SharedPtr &node, const std::string &config_key, const std::string &name_space,
  ...
  // Load failures from this call are intentionally unchecked below: unlike GNSS/Generic, none of this handler's fields
  // are compulsory, so a missing file just means "use code defaults", which is expected for a fresh instance with no
  // plugin-specific overrides. A malformed file still logs an RCLCPP_ERROR from within addYamlFile() itself.
